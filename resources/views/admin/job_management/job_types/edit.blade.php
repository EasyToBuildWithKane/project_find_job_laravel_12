...
@section('action', 'Chỉnh sửa loại công việc')
@section('admin-content')
<div class="container-fluid">
    <div class="card shadow-sm border-0">
        <div class="card-body">
            <form action="{{ route('admin.job_types.update', $jobType->id) }}" method="POST" id="jobTypeForm">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="name" class="form-label">Tên loại công việc</label>
                    <input type="text" name="name" class="form-control" id="name" value="{{ old('name', $jobType->name) }}">
